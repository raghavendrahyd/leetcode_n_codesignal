# Description: A string is said to be beautiful if b occurs in it no more times than a; c occurs in it no more times than b; etc.
...
def solution(inputString):
    ...
    if "a" not in dict2_keys:
        return False
    for i in range(len(dict2_keys) - 1):
        if ord(dict2_keys[i + 1]) - ord(dict2_keys[i]) != 1:
            return False
